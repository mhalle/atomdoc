...
class TextNode_M:
    value: str = ""
    children: Array["TextNode_M"] = []


def make_doc():
    return Doc(root_type="TextNode_M", nodes=[TextNode_M])


def text(doc, *values):
    nodes = []
    for v in values:
        n = doc.create_node(TextNode_M, value=v)
        nodes.append(n)
    return nodes
...
class TestBase:
    def test_node_from_different_doc_should_throw(self):
        doc1 = make_doc()
        doc2 = make_doc()
        node1 = doc1.create_node(TextNode_M)
        with pytest.raises(RuntimeError, match="different document"):
            with doc2.transaction():
                doc2.root.children.append(node1)

    def test_duplicate_node_id_should_throw(self):
        doc = make_doc()
        node1 = doc.create_node(TextNode_M)
        with doc.transaction():
            doc.root.children.append(node1)
...
class TestAppend:
    ...
    def test_append_multiple(self):
        doc = make_doc()
        with doc.transaction():
            doc.root.children.append(*text(doc, "1", "2", "3", "4"))
...
class TestPrepend:
    def test_prepend_single(self):
        ...
        with doc.transaction():
            ...
            doc.root.children.prepend(*text(doc, "0"))
        assert_doc(doc, ["0", "1", "2"])

    def test_prepend_multiple(self):
        doc = make_doc()
        with doc.transaction():
            doc.root.children.append(*text(doc, "3", "4"))
            doc.root.children.prepend(*text(doc, "1", "2"))
        assert_doc(doc, ["1", "2", "3", "4"])

    def test_prepend_to_empty(self):
        doc = make_doc()
        with doc.transaction():
            doc.root.children.prepend(*text(doc, "1"))
        assert_doc(doc, ["1"])
...
class TestInsertBefore:
    def test_before_middle(self):
        doc = make_doc()
        with doc.transaction():
            doc.root.children.append(*text(doc, "1", "3"))
        with doc.transaction():
    ...
    def test_before_first(self):
        doc = make_doc()
        with doc.transaction():
            doc.root.children.append(*text(doc, "2", "3"))
        with doc.transaction():
    ...
    def test_before_multiple(self):
        doc = make_doc()
        with doc.transaction():
            doc.root.children.append(*text(doc, "1", "4"))
        with doc.transaction():
    ...
    def test_insert_before_root_raises(self):
        doc = make_doc()
        with pytest.raises(RuntimeError, match="no parent slot"):
            with doc.transaction():
...
class TestDelete:
    ...
    def test_delete_removes_from_node_map(self):
        doc = make_doc()
        with doc.transaction():
            n = doc.create_node(TextNode_M, value="x")
...
class TestRangeMove:
    ...
    def test_move_range_before(self):
        ...
        with doc.transaction():
            n3.to(n4).move(n1, "children", "prepend")
        assert_doc(doc, ["1", "2"])
        assert [c.value for c in n1.children] == ["3", "4"]
    ...
    def test_move_to_self_raises(self):
        ...
        with pytest.raises(ValueError, match="Target is in the range"):
            with doc.transaction():
                n1.to(n1).move(n1, "children")

    def test_move_to_descendant_raises(self):
        doc = make_doc()
        with doc.transaction():
            parent = doc.create_node(TextNode_M, value="parent")
            child = doc.create_node(TextNode_M, value="child")
            doc.root.children.append(parent)
            parent.children.append(child)
        with pytest.raises(ValueError, match="descendant"):
            with doc.transaction():
                parent.move(child, "children")
...
class TestReplace:
    def test_replace_single(self):
        ...
        n2 = doc.root.children[1]
        with doc.transaction():
            replacement = doc.create_node(TextNode_M, value="NEW")
...
class TestMixedOperations:
    def test_delete_and_reinsert_same_transaction(self):
        ...
        doc = make_doc()
        with doc.transaction():
            n1 = doc.create_node(TextNode_M, value="1")
            doc.root.children.append(n1)
            n1.delete()
            doc.root.children.append(n1)
        assert_doc(doc, ["1"])
    ...
    def test_insert_multiple_levels(self):
        doc = make_doc()
        with doc.transaction():
            p = doc.create_node(TextNode_M, value="parent")
            c1 = doc.create_node(TextNode_M, value="child1")
            c2 = doc.create_node(TextNode_M, value="child2")
            g1 = doc.create_node(TextNode_M, value="grandchild1")
            doc.root.children.append(p)
            p.children.append(c1, c2)
    ...
    def test_delete_parent_removes_descendants_from_map(self):
        ...
        with doc.transaction():
            p = doc.create_node(TextNode_M, value="parent")
            c = doc.create_node(TextNode_M, value="child")
            doc.root.children.append(p)
            p.children.append(c)
        pid, cid = p.id, c.id
        with doc.transaction():

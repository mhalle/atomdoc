...
import pytest
from atomdoc import Doc, AtomNode, Array, node


@node
class TextTr:
    value: str = ""
    children: Array["TextTr"] = []


def make_doc():
    return Doc(root_type="TextTr", nodes=[TextTr])


def text(doc, *values):
    return [doc.create_node(TextTr, value=v) for v in values]


def setup_tree():
    """Create doc with root -> [1, 2, 3, 4]; 1->[1.1, 1.2]; 3->[3.1]."""
    doc = make_doc()
    with doc.transaction():
        doc.root.children.append(*text(doc, "1", "2", "3", "4"))
        doc.root.children[0].children.append(*text(doc, "1.1", "1.2"))
        doc.root.children[2].children.append(*text(doc, "3.1"))
    return doc
...
class TestNavigationProperties:
    def test_parent(self):
        ...
        assert doc.parent(n1) is doc.root
        assert doc.parent(doc.root) is None

    def test_prev_sibling(self):
        doc = setup_tree()
        n1, n2 = doc.root.children[0], doc.root.children[1]
        assert doc.prev_sibling(n1) is None
        assert doc.prev_sibling(n2) is n1

    def test_next_sibling(self):
        doc = setup_tree()
        n1, n2 = doc.root.children[0], doc.root.children[1]
        n4 = doc.root.children[3]
        assert doc.next_sibling(n1) is n2
        assert doc.next_sibling(n4) is None
...
class TestDescendants:
    def test_depth_first(self):
        doc = setup_tree()
        vals = [n.value for n in doc.descendants(doc.root)]
        assert vals == ["1", "1.1", "1.2", "2", "3", "3.1", "4"]

    def test_excludes_self(self):
        doc = setup_tree()
        n1 = doc.root.children[0]
        vals = [n.value for n in doc.descendants(n1)]
        assert vals == ["1.1", "1.2"]

    def test_empty_for_leaf(self):
        doc = setup_tree()
        n4 = doc.root.children[3]
        assert list(doc.descendants(n4)) == []
...
class TestAncestors:
    def test_walk_to_root(self):
        ...
        ancestors = list(doc.ancestors(n1_1))
    ...
    def test_excludes_self(self):
        doc = setup_tree()
        n1 = doc.root.children[0]
        ancestors = list(doc.ancestors(n1))
        assert n1 not in ancestors

    def test_root_has_no_ancestors(self):
        doc = setup_tree()
        assert list(doc.ancestors(doc.root)) == []
...
class TestNextSiblings:
    def test_forward_from_first(self):
        doc = setup_tree()
        n1 = doc.root.children[0]
        vals = [n.value for n in doc.next_siblings(n1)]
        assert vals == ["2", "3", "4"]

    def test_excludes_self(self):
        doc = setup_tree()
        n1 = doc.root.children[0]
        assert n1 not in list(doc.next_siblings(n1))

    def test_empty_for_last(self):
        doc = setup_tree()
        n4 = doc.root.children[3]
        assert list(doc.next_siblings(n4)) == []
...
class TestPrevSiblings:
    def test_backward_from_last(self):
        doc = setup_tree()
        n4 = doc.root.children[3]
        vals = [n.value for n in doc.prev_siblings(n4)]
        assert vals == ["3", "2", "1"]

    def test_excludes_self(self):
        doc = setup_tree()
        n4 = doc.root.children[3]
        assert n4 not in list(doc.prev_siblings(n4))

    def test_empty_for_first(self):
        doc = setup_tree()
        n1 = doc.root.children[0]
        assert list(doc.prev_siblings(n1)) == []
...
class TestRange:
    ...
    def test_invalid_range_raises(self):
        ...
        with pytest.raises(ValueError, match="not a later sibling"):
            list(n4.to(n1))
    ...
    def test_cross_parent_range_raises(self):
        ...
        doc = make_doc()
        with doc.transaction():
            p1 = doc.create_node(TextTr, value="p1")
            p2 = doc.create_node(TextTr, value="p2")
            doc.root.children.append(p1, p2)
            p1.children.append(*text(doc, "c1"))
            p2.children.append(*text(doc, "c2"))
...
class TestIsinstance:
    def test_isinstance_works(self):
        ...
        for child in doc.root.children:
            assert isinstance(child, TextTr)
            assert isinstance(child, AtomNode)

    def test_isinstance_root(self):
        doc = setup_tree()
        assert isinstance(doc.root, TextTr)
